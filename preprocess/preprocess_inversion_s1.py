...
def reform_era5(params):
    ...
    """
    era5_swh_path = params['data_path']['era5_SST']
    ...
    for file in natsorted(os.listdir(era5_ori_path)):
        ...
        era5_ori_swh = era5_ori_data.variables['sst'][:].data
        

        for i in range(len(era5_time)):
            era5_swh = np.array(era5_ori_swh[i], dtype=np.float32)[:-1, :-1]
            # era5_swh = np.where(era5_swh == np.nan, 0, era5_swh)
...
def create_sample_one_file(h8_file, h8_ori_path, era5_uv_path, 
    ...
    temp_era5_file = os.path.join(era5_uv_path, h8_file.split('_')[2][0:4])
...
def create_sample(params):
    ...
    sub_dir = ['train','valid','test'] #, 'valid', 'test'
        ...
        for year in temp_time_range:
            ...
            for month in months:
                ...
                for h8_file in h8_files:
                    create_sample_one_file(h8_file, temp_h8_ori_month_path, era5_swh_path, 
